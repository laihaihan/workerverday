(function($){
	$.fn.flow = function(option){
		var defaultOptions = {
			...
			handle : "draw",
			img : {
				right : top.appPath + "/core/js/flow/images/right_sword.png",
				up : top.appPath + "/core/js/flow/images/right_sword.png"
			},
			hover : function(){
		...
		return this.each(function(){
			...
			var drawLine = function(start,end){
				...
				if(x2 == y2){
					...
					jg_draw.drawImage(opts.img.right,y1-3,y2-4,5,10);
				}else{
					if(x2 < y2){
						...
						jg_draw.drawImage(opts.img.up,y1-3,y2-4,5,10);
					}else{
			...
			var drawDiv = function(objectList,spanID,left,top,maxHeight){
				...
				for(var i = 0; i < objectList.getSize();i++){
					objectList.child[i].id = "'"+objectList.child[i].id+"'";
