...
<head>
	...
	function search(){
		...
		window.frames["chartIframe"].location="<%=request.getContextPath()%>/jsp/chart/bjqs/bjqs4Report.jsp?area="+area+"&dept="+dept+"&starttime="+starttime+"&endtime="+endtime;
	}
																...
																<TBODY>
																	<TR>
																		<TD vAlign="top" height="450" align="center" bgcolor="#F1F4F5">
																			  ...
																			  <iframe id="chartIframe" name="chartIframe" src="<%=request.getContextPath()%>/jsp/chart/bjqs/bjqs4Report.jsp?area=<%=defultArea %>&dept=<%=defultDept %>&starttime=<%=starttime %>&endtime=<%=endtime %>" width="100%" height="100%"scrolling="no"></iframe>														
																		</TD>
																	</TR>
																</TBODY>
															</TABLE>
