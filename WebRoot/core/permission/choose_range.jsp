...
<%
	//显示类型 0=人员，1=部门，2=职位，3=角色，4=环节，5=扩展功能
	...
	String condition ="&appUnid="+appUnid+"&flowUnid="+flowUnid;
	fn="getPersion";
...
<body>
	<div id="form_content" style="width:99.5%; height:100%;">
		...
		<div style="height:25px;" >
			...
			<div class='choose_type' style="float: left;width:80%; text-align:left;height:25px;border-bottom:none;border-left:none;";>
				<label ><input type="radio" id="type" name="type" value="0" ${type eq '0'?'checked="checked"':''} onclick='changeType(this.value);'>人员</label>
				<label ><input type="radio" id="type" name="type" value="1" ${type eq '1'?'checked="checked"':''} onclick='changeType(this.value);'>部门</label>
				<label ><input type="radio" id="type" name="type" value="2" ${type eq '2'?'checked="checked"':''} onclick='changeType(this.value);'>职位</label>
				<label ><input type="radio" id="type" name="type" value="3" ${type eq '3'?'checked="checked"':''} onclick='changeType(this.value);'>角色</label>
				<%if(!StrUtil.isNull(flowUnid)){ %>
				<label ><input type="radio" id="type" name="type" value="4" ${type eq '4'?'checked="checked"':''} onclick='changeType(this.value);'>环节</label>
				<%} %>
				<label ><input type="radio" id="type" name="type" value="5" ${type eq '5'?'checked="checked"':''} onclick='changeType(this.value);'>扩展功能</label>
			</div>
		</div>
	...
	var choosePersonnel = {
		...
		submit:function(){
			...
			top.$("#i_"+domId).contents().find("#<%=returnType%>").val(type);
			top.popup.close()
		}
	}
	...
	$(function(){
		var setting = {
			...
			async: {
				enable: true,
				url:"${path}/resource.action?fn=<%=fn%>&isShowAll=<%=isShowAll+condition%>",
				autoParam:["id"],
				dataFilter: filter
			},
			callback:{
