...
try{
	...
	//webFXTreeConfig.loadingIcon = "../ui_style/style_1/img/xtree/loading.gif";
	webFXTreeConfig.loadingIcon = imgPath + "loading.gif";
	//add by jc_seekart
	webFXTreeConfig.chkbox = true;
	webFXTreeHandler.persistenceManager.cookieName = "linewell_xtree";
}catch(e){}
...
WebFXLoadTree._attrs = ["text", "src", "action", "id", "target", "checked"];
...
WebFXLoadTree.loadXmlDocument = function (jsNode) {
	...
	//jsNode._xmlHttp.setRequestHeader("charset", "utf-8");
...
WebFXLoadTree.documentLoaded = function (jsNode) {
	...
	//modify by sxh 2007-8-7	����jsNode.src="***.asp��֮������xml�ı�
	//alert(jsNode._xmlHttp.responseXML.xml);
	//alert(jsNode._xmlHttp.responseText);
	//alert(jsNode._xmlHttp.responseXML.xml);
	if(!jsNode._xmlHttp.responseXML.xml){
		var doc = new ActiveXObject("Msxml2.DOMDocument.4.0");
		doc.async = false;
		doc.loadXML(jsNode._xmlHttp.responseText);
	}else{
		var doc = jsNode._xmlHttp.responseXML;
	}
	...
	if (jsNode.errorText != "") {
		...
		//add by jc�����ӽڵ��ɾ��ڵ�,���ڲ��������û����������û��б�
		//jsNode.getParent().remove(jsNode);
	} else {
		// remove dummy
	...
	if (count == 0) {
		//获取image左边的对象
		var objImg=document.getElementById("chk_"+jsNode.getId());
		//获取image对象
		var obj = objImg.nextElementSibling || objImg.nextSibling;
		//当没有子节点的时候（count==0）重设节点图标
		obj.src=webFXTreeConfig.folderIcon;
	}
};
