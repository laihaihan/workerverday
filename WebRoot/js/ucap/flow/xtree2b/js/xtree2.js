...
//var imgPath = "../ui_style/style_1/img/xtree/";
//防止模式对话框中取值错误 mdf by jc 20100618
var sSystemPath ="/"+location.pathname.split("/")[(location.pathname.indexOf("/")>0?0:1)] +"/";
var isMulti;
var imgPath = sSystemPath + "js/ucap/flow/xtree2b/images/";
...
_p.cookieName = "lwjc-tree-cookie-persistence"
...
var webFXTreeConfig = {
	rootIcon        : imgPath + "folder.png",
	openRootIcon    : imgPath + "openfolder.png",
	folderIcon      : imgPath + "folder.png",
	openFolderIcon  : imgPath + "openfolder.png",
	fileIcon        : imgPath + "file.png",
	iIcon           : imgPath + "I.png",
	lIcon           : imgPath + "L.png",
	lMinusIcon      : imgPath + "Lminus.png",
	lPlusIcon       : imgPath + "Lplus.png",
	tIcon           : imgPath + "T.png",
	tMinusIcon      : imgPath + "Tminus.png",
	tPlusIcon       : imgPath + "Tplus.png",
	plusIcon        : imgPath + "plus.png",
	minusIcon       : imgPath + "minus.png",
	blankIcon       : imgPath + "blank.png",
...
var webFXTreeHandler = {
	...
	getUniqueId: function () {
		return this.persistenceManager.cookieName+this.idPrefix + this.idCounter++;
	},
	all: {},
...
_p.setChecked = function (bChecked) {
	...
	if (this.loaded) {
		...
		if (bChecked != this.getChecked())
	    {
			...
			webFXTreeHandler.all[this.id]._checked = inputEl.checked = bChecked;
	...
	//alert(inputEl.checked);
};
//add by jc_seekart
_p.setDBChecked = function (bChecked)
{
	//alert(!this.loading);
	if (this.src)this.expand();
	if (this.loaded) {
...
//add by jc_seekart
_p.setDDBChecked = function (bChecked)
{
	if (this.src)this.expand();
	if (this.loaded) {
		this.setChecked(bChecked);
		...
		for(var i=0; i<oChildItems.length; i++)
		{
			...
			var oDiv = document.getElementById(oid);
			var oInput = oDiv.getElementsByTagName("INPUT")[0];
	        if(oChildItems[i].hasChildren())
	        {
				oChildItems[i].expand();
	            if(oInput != null){
	                oChildItems[i]._checked = oInput.checked = bChecked;
					webFXTreeHandler.all[oid]._checked = oInput.checked = bChecked;
				}
...
_p.getRowHtml = function () {
	...
	return "<div class=\"" + this.getRowClassName() + "\" style=\"padding-left:" +
		...
		//add by jc_seekart
		(webFXTreeConfig.chkbox ? this.getChkHtml() : "")+
		//this.getChkHtml() +
		this.getIconHtml() +
		this.getLabelHtml() +
		//"</span>" +
...
//add by jc_seekart
_p.getChkHtml = function () {
	return "<input id=chk_"+this.id+" class=\"tree-check-box\" type=checkbox "+
	(this._checked ? " checked=\"true\"" : "") +
	" ondblclick=\"webFXTreeHandler.all[this.parentNode.parentNode.id].setDBChecked(this.checked);\" "+
	" onclick=\"webFXTreeHandler.all[this.parentNode.parentNode.id].setDBChecked(this.checked);\">";
};
...
//modified by csj 2008.10.15
	var el = e.target || e.srcElement;
	  
	val = el.parentNode.parentNode.id;
	
	if( !checkValExist( $( "SingleSelResult"), val ) ){
		
		
		if(val.length ==32){
			if( isMulti == '0' ){ 
				$( "SingleSelResult").options.length = 0;
			}
			addOption( $( "SingleSelResult"), val, el.innerHTML );
		}
...
_p.getEventHandlersHtml = function () {
	return " onclick=\"if(event.srcElement.tagName != 'INPUT')return webFXTreeHandler.handleEvent(event)\" " +
		"onmousedown=\"return webFXTreeHandler.handleEvent(event)\" " +
		"ondblclick=\"if(event.srcElement.tagName != 'INPUT')return webFXTreeHandler.handleEvent(event)\" " +
