...
var ucapCommonFun = {
	...
	autoMenuHeight : function() {
		...
		//try{
			...
			if($("ucapSystemButtonListBox")){//分级管理自适应
				ucapSession.middleHeight = ucapSession.middleHeight - parseInt($("ucapSystemButtonListBox").clientHeight-5);
				if($("ucapMainLeft"))$("ucapMainLeft").style.height = ucapSession.middleHeight - parseInt($("ucapSystemTreeMenu").clientHeight) +"px";
			...
			if (Ext.getDom("leftArrowhead")) {
				Ext.getDom("leftArrowhead").style.height = (ucapSession.middleHeight) + "px";
				Ext.getDom("leftArrowhead").style.paddingTop = ((ucapSession.middleHeight)/ 2 - 20)+ "px";
			}
			//部门树
...
/*			try{
				...
				if(gridDiv){
					...
					gridDiv.style.height = ght-cint-20;
				}
			}catch(e){}*/
	...
	setIframeViewHeight:function(){
		...
		if(ifr){
			...
			viewTabs.tabs.setHeight(0);
			viewTabs.tabs.setHeight(height);
			var item=viewTabs.tabs.getActiveTab();
			var frameID = 'ifr'+item.id;
			var ifrm =document.frames?document.frames[frameID]:
              				document.getElementById(frameID).contentWindow;
			if(ifrm && ifrm.ucapSession){
				ifrm.ucapSession.middleHeight=height-cint; //设置iframe窗口中 模块的高度为父模块高度-25
				ifrm.view.setGridHeight(); //设置视图的相关高度
				//设置iframe的高度						
				document.all('ifr'+item.id).style.height =height-cint; 
			}
		}
	},
	setIframeViewWidth:function(width){
		...
		if(ifr){
			...
			for(var i=0;i<ifr.length;i++){
				var srcUrl = ifr[i].getAttribute("src");
				if(srcUrl.indexOf("sys/system/index.jsp")>-1 || srcUrl.indexOf("sys/jsp/view.jsp")>-1){
					if (ifr[i].contentWindow.ucapCommonFun){
						ifr[i].style.width = width;
						ifr[i].contentWindow.ucapCommonFun.setViewWidth(width-2);;
					}
				}
			}
		}
	...
	refreshParentView : function() {
		...
		if (window.parent) {
			...
			// 代表是div 窗口打开
			if (window.parent.ucapCurOpenDocViewId != ""
					&& typeof window.parent.ucapCurOpenDocViewId != "undefined") {
				var iframeView = window.parent.Ext.getDom("ifr"
						+ window.parent.ucapCurOpenDocViewId);
				if (iframeView) {
	 ...
	 */
	moduleMenuClk : function(id, type, openType, title,purl,moduleUnid) {
		...
		if(ucapHeader.openStyle==1 || ucapHeader.openStyle=="01"){//判断是否是页签风格
			if(id && id.indexOf("ucapTitle")>-1){
				title = id.replace(/.*([?&]ucapTitle=)/ig,"").split("\&")[0];
			}	
			var tabid = id.replace(/.*([?&]unid=)/ig,"").split("\&")[0];
			if (tabid=="") { 
				tabid=id.replace(/\W/g,"");//以url作为ID
			} else {
				id +="&isTab=1";
			}
			if (openType == "02" || openType == 2 ) {//判断是否是当前窗口打开
				...
				viewTabs.commonAddTab(ucapSession.ucapViewId,id,1,tabid,title);		
			} else {//以iframe方式打开
				viewTabs.commonAddTab(ucapSession.ucapViewId,id,2,tabid,title);	
			}
		} else {//是普通页面方式
			if (openType == "02" || openType == 2 ) {//判断是否是当前窗口打开
				...
				//当前iframe打开
				if(id && id.indexOf("document.jsp")==-1)
	...
	indexOpen : {
		 ...
		 */
		openView : function(id, isMenu, title,purl,bModuleUnid) {
			...
			if (ucapSession.viewOpenType == 1) {
				initView(id, ucapSession.ucapViewId, title, true,purl,bModuleUnid);
			} else {
				initView(id, ucapSession.ucapViewId,"",false,purl,bModuleUnid);
			}
		},
		 ...
		 */
		newDocument:function(formType,formId,height,width,purl){
			 //无流程的打开
			var url = ucapSession.appPath +"sys/jsp/";
			url+="document.jsp?unid="+"&type="+formType+"&formId="+formId+"&"+(purl||"");
			ucapCommonFun.ucapOpenDoc(url,0,"",height,width);
		}
	},
	/**
	 * 新建流程的函数 没有参数（全部流程）
	 ...
	 * @param {}
	 *            type 打开类型 0 表示 新窗口，1表示当前窗口 2表示div方式 如果为空，则默认以 界面方案中的配置类型进行打开
	 * @param {}
	...
	buttonFun : {
		...
		modifyUser : function(unid, deptUnid) {
			window
					.open(ucapSession.appPath
							+ "sys/jsp/document.jsp?unid="
							+ unid
			...
			for (fkey in json) {
				...
				if (o && json[fkey] && Ext.type(json[fkey]) == 'object') {
					try {
						...
						if (Ext.type(o) != "array") {
							if (!applyIf)
								Ext.applyIf(o, json[fkey]);
							else
								Ext.apply(o, json[fkey]);
						} else {
							o = o[0];
						}
						// 选择框、赋值
							...
							case 'input' : {
								switch (o.getAttribute("type").toLowerCase()) {
									...
									default : {
										...
										//modify by jc 新文档时HTML页面有默认值的时候不覆盖HTML上value的默认值
										if(o.getAttribute("sourceType")=="01"){
											//新文档及手工输入时以页面的默认值为准
											if(o.value==""){
												o.value=_v;
											}else{
										...
										//o.value = _v;
										//进行input特效的的绑定
										if(!o.getAttribute("id"))o.setAttribute("id",o.getAttribute("name"));
 ...
 * 
 */
function gotoVisualFlow(isRate){
	if("undefined"!=typeof(isRate) && isRate){
		window.open(hostPath+appPath+"sys/jsp/visualflow.jsp?appUnid="+ucapManagerTree.curBelongToAppId );
	}else{
		window.open(hostPath+appPath+"sys/jsp/visualflow.jsp?appUnid="+ucapHeader.appUnid);
	}
}/**
 * 菜单定制 模块定制方法
 * @author yjy
...
var _UcapForm = {
	...
	cfg : {
		...
		htmlTmpStr:{
			...
			btn_cn_html:'<input id="{id}_Cn_" type="text" class="{class}" readOnly/>&#160;<input id="btn_{id}" class="btnOnlyChannel" value="{text1}" type="button"/>',
			areabtn_html:'<textarea id="{id}_Cn_" class="{class}" readOnly row="2"></textarea>&#160;<input id="btn_{id}" class="btnOnlyChannel" value="{text1}" type="button"/>',
		...
		toolBarBtnTmp:{
			...
			btn:'<button title="{tip}" onclick="{code}" class=btnn1 onmouseover="this.className=\'btnn3\'" onmouseout="this.className=\'btnn1\'">\
					<IMG src="'+ucapHeader.sUserStylePath+'ucapimages/default_btn.gif" align=absMiddle>&nbsp;{name}</button>',
...
_UcapForm.tool = {
	...
	embellishForm:function(args,dictTree){
		...
		var fn20 = function(_s,s02){
			...
			if(s02){//视图列
				...
				Ext.apply(btn,{"onclick":function(){
					var _ev=obj.getAttribute("Evalue")||"";//回调函数
					var _sWidth=obj.getAttribute("sWidth")||"";
					var _sHeight=obj.getAttribute("sHeight")||"";
					//sTitle,purl,recordSplit,colSplit
					var _sTitle=obj.getAttribute("sTitle")||"";
					var _purl=obj.getAttribute("purl")||"";
					var _recordSplitl=obj.getAttribute("recordSplit")||"";
					var _colSplit=obj.getAttribute("colSplit")||"";
					return (function(){selectView(_ss,fieldNames,nameEn,_t,_ev,_sTitle,_purl,_recordSplitl,_colSplit,_sWidth,_sHeight);})();
				}});
			}else{//通用选择框、字典选择框
				if(_t!=1){
		...
		switch(sourceType){
			...
			case "05":{//日期时间
				source = source||"yyyy-MM-dd HH:mm:ss";
				var img = '<span><IMG style="CURSOR: pointer;" onclick="{onclick}" src="'+ucapSession.appPath+'js/ucap/calendar/skin/datePicker.gif" align=absMiddle/></span>';
				complieTmp(img,Ext.getDom(nameEn).parentNode,{"onclick":"WdatePicker({el:'"+nameEn+"',dateFmt:'"+source+"'});"});
				break;
			}
			...
			case "09":{//文件上传
				...
				if((cfg.isRead+"")=="1"){
					if(!item.value[0]){
						...
						filecfg = {"id":nameEn,"class":"inputred","display":"none","downFile":'<span onclick="_UcapForm.tool.valFile(\''+nameEn+'\');" style="color:red;cursor:hand;">重新上传</span>&#160;&#160;' +
		...
		var isChinese = (obj && obj.getAttribute("chinese"))?obj.getAttribute("chinese"):item["chinese"];
		if(isChinese){
	...
	form : function(fjson,ijson){
		...
		//modify by jc 20091130 nameEn取表单信息不取显示表单或组合表单信息
...
_UcapForm.saveRefreshTree=function(){
	if(window.opener){
		...
		if(ifr){
			for(var i=0;i<ifr.length;i++){
				if(ifr[i].getAttribute("src").indexOf("sys/system/index.jsp")>-1){
 ...
 */
_UcapForm.simpleDocSave= function(saveCallBack,msgObj){
	var re = _UcapForm.docSave(saveCallBack,"1",msgObj);
	return re;
};
/**
 * 保存表单 
 ...
 *  modifyMsg【无需保存】信息内容，saveMsg【保存提示】信息内容
 */
_UcapForm.docSave = function(saveCallBack,simple,msgObj){
			...
			if((ifr_src=="#" && iframeFlag.indexOf("true")==-1 && fileValue!="")){
				if(iframe.getAttribute("flag")!="true"){
					Ext.get("iframe"+inputId).on("load",function(){
							...
							_UcapForm.docSave(saveCallBack,simple,msgObj);
						});
					try{
	...
	var requestConfig = {
		...
		callback:function(options,success,response){
			...
			if(msgObj.isSaveMsg)curwin.Ext.Msg.alert("保存提示",msgObj.saveMsg||flag);
		}
	}
				...
				if(!window.parent.viewTabs.tabs){
					...
					try{
						...
						if(!murl){//默认用视图打开
							...
							}else if((murl||"").indexOf("/document.jsp")==-1){
								//根据URL打开
	...
	returnForm.selectTag = function (n,selfObj){ 
		...
		try{
			...
			for(var i=0;i<types.length;i++){
				var t = types[i];
				var divId = (!contents[i] ||t=="08")?oDiv[n].id:pfx_ucap+contents[i];
				...
				switch(t){
					case "03" :{//视图
						...
						//将URL上需要覆盖的参数清空
						var parm = location.href.replace(/(^[^?]*\?)|(&fformUnid=)[\w]*|(&funid=)[\w]*|([&]?unid=)[\w]*/ig,"");
						//alert(parm+"&fformUnid="+cfg.mainUnid+"&funid="+unid);
						//非第一次打开时加载视图数据,组装新的参数
						if(oFlwDiv.innerHTML=="")
							initJspView(contents[i],divId,"unid="+unid+"&"+parm+"&fformUnid="+cfg.mainUnid+"&funid="+unid,true,true,true);
						break;
					}
					case "04"://日志
						break;
					...
					case "07"://打開JSP
						...
						var reload=ucapCommonFun.isReload(sUrl);
					...
					case "10":{//表单HMTL配置
						_UcapFormFun.modifyHTML(oFlwDiv.firstChild);
						break;
					}
		...
		var win = new Ext.Window({
		    ...
		    buttons: [{
		        ...
		        handler:function(){
					...
					types = types.replace(/(~!~)$/,"");
					types = escape(types);
					var isDocSave = ucapCommonFun.getUrlParameter("unid")?"1":"0";
...
var _UcapFormFun = {
	...
	getHtmlFile:function(isRead){
		var belongToAppId = ucapCommonFun.getUrlParameter("belongToAppId")||"";
		var htmlFolder  = _UcapForm.cfg.defaultPath;
	 	var nameEn = $("form_name_en");
		if(nameEn && nameEn.value.toLowerCase().indexOf("ucap_")==0){
			htmlFolder=_UcapForm.cfg.systemPath;
		}
		var sform = $("sform_html_url"),filename;
		if(sform && sform.value){
			filename = sform.value;
		}else{
			filename = ucapCommonFun.getUrlParameter("unid") + ".html"
		}
		var sUrl = htmlFolder + (isRead?"R_":"") + filename;
		return sUrl;
	},
	/**
	 * 获取webEditor对象
	...
	createHTML : function(){
		...
		var belongToAppId = ucapCommonFun.getUrlParameter("belongToAppId")||"";
		var tableName = $V("form_name_en"),filePath;
		...
		var requestConfig = {
			url:ucapSession.baseAction,
			params:{"type":"CreateHtmlAction","unid":formId,"belongToAppId":belongToAppId,"filePath":filePath},
			//jsonData:json,
			callback:function(options,success,response){
				if(success){
					var exjson = Ext.decode(response.responseText);
	...
	initChannelId:function(){
		...
		if(enObj.value && enObj.value!="undefined"){
			...
			cnObj.value = enObj.value = ((window.opener&&window.opener.top)||window.parent)._UcapForm.cfg.unid;
		}
	},
	/**
	 * 设计管理系统界面方案-首页类型
...
var view={
	...
	createGridDiv:function(index,render){
		...
		searchNewDivHtml+="</select> <input type=\"button\" value=\"搜索\" id=\"simpleSearch_"+index+"\" class=\"btn1\" onclick=view.search("+index+")></div><div style=\"float:left;padding:5px;\"> <input type=\"button\" class=\"btn1\" id=\"advancedSearch_"+index+"\" onclick=\"view.setSearchMore(this,"+index+")\" value=\"▼高级搜索\" /></div><div>";
...
//		mainRight.appendChild(formDiv);
	...
	setSearchMore:function(obj,index){
    	...
    	if(searchMoreDiv.style.display==''){  
    		...
    		if(view.viewBaseInfos[index].viewType=="02" || view.viewBaseInfos[index].viewType=="03"){
    			...
    			gridPanel.style.height=gridPanel.offsetHeight+searchMoreDiv.offsetHeight;
    		}else{
    			gridPanel = Ext.getCmp(view.namePrefix+index);
    			gridPanel.setHeight(gridPanel.getInnerHeight()+searchMoreDiv.offsetHeight+54);
    		}
    		...
    		if(view.viewBaseInfos[index].viewType=="02" || view.viewBaseInfos[index].viewType=="03"){
    			...
    			gridPanel = Ext.getCmp(view.namePrefix+index);
    			innerWidth = gridPanel.getInnerWidth();
    			gridPanel.setHeight(gridPanel.getInnerHeight()-divHeight+54);
    		}
    		searchMoreDiv.style.width =innerWidth-2;
    		obj.value = "▲高级搜索";
    	}
    },
	...
	loadCommonGrid:function(curViewId,index,noPreview,noSelfConfig){
		...
		if(view.viewBaseInfos[index].isEdit){
			...
			items =Ext.util.JSON.decode(view.getColumnModuleJson(view.json,index));
		}
	...
	loadOtherGrid:function(curViewId,index,pdid,noPreview,noSelfConfig){
		...
		var cint = ucapSession.viewOpenType=="01"?50:0;
		var ght = ucapSession.middleHeight - cint;
		if("undefiend"!=searchDiv && null!=searchDiv){
			ght=ght-searchDiv.offsetHeight;
		}
			...
			for(var i=0;i<records.length;i=i+4){
				...
				for(var u=0;u<4;u++){
					...
					if(null==vdata){
						...
						for(var j in vdata){
							if(k==1){
								imageHtml+="<A href=\"#\"><img height=100 width=154 src=\""+vdata[j]+"\"></A>";
							}else if(k==2){
	...
	insertEditorGridData:function(insertDataJson){
		...
		for(var j=beginIndex;j<colCount;j++){
			...
			if(dataIndex=="~display~opr~")continue;
	...
	getEditorGridDatas:function(){
		...
		for(var i=0;i<rowCount;i++){
			...
			datas+="formUnid:'";
			if(view.viewBaseInfos[view.index].sourceType=="01"){
				datas+=view.viewBaseInfos[view.index].sourceUnid;
			}else{
				datas+=view.viewBaseInfos[view.index].formId;
			}
			...
			for(var j=beginIndex;j<colCount;j++){
				...
				if(dataIndex==unidIndex || dataIndex=="~display~opr~")continue;
				//不是新增并且没有过的话，直接跳过
	 ...
	 */
	openViewDoc:function(unid,formType,formId,isFlowItem,tmpUrl,openJs,height,width,viewId,openST){
		...
		if(undefined!=openJs && openJs!=""){
			...
			if (openJs.indexOf("OpenFlowDocByTodoUnid")>-1){
				js = openCode+"('"+unid+"',ucapCommonFun.ucapOpenDoc,1"+par+"&viewMId="+viewId+"&openST="+openST+")";	
			} else{
			...
			if(isFlowItem){
				...
				try{
			    		...
			    		var belongToAppId = ucapManagerTree.curBelongToAppId ;
				    	if(belongToAppId){
							url+="document.jsp?unid="+unid+"&belongToAppId="+belongToAppId+"&type="+formType+"&formId="+formId+"&viewMId="+viewId+"&openST="+openST;
						} else{
							url+="document.jsp?unid="+unid+"&type="+formType+"&formId="+formId+"&viewMId="+viewId+"&openST="+openST;
						}
				}
				catch(e){
						url+="document.jsp?unid="+unid+"&type="+formType+"&formId="+formId+"&viewMId="+viewId+"&openST="+openST;
	 ...
	 */
	refresh:function(){
		if(null!=view.viewBaseInfos && view.viewBaseInfos[view.index].viewType!="02" && view.viewBaseInfos[view.index].viewType!="03"){
			try{
				var grid = Ext.getCmp(view.namePrefix+view.index);
				if (grid){
			...
			if(null!=view.stores && null!=view.stores[view.index])view.stores[view.index].reload();
		}
	},
	...
	advancedSearch:function(index){
		...
		for(var i=0;i<inputObjs.length;i++){
			var inputObj = inputObjs[i];
			var inputObjName = inputObj.name.replace(view.advPrefix,"");
			...
			if((inputObj.type=="text"||inputObj.type=="hidden") && inputObj.value!=""){
				if(view.hasBeginInputName(inputObjName,index)){				
					...
					if(inputObjName.lastIndexOf("_Cn_")>0){
						continue;
					}
...
var viewTabs={
	 ...
	 * @param {} index
	 */
	commonAddTab:function(divid,url,type,tabid,title,index){
		...
		if (type==2) {//说明是iframe页面
			html = "<iframe name='ifr"+tabid+"' id='"+tabid+"' src='"+url+"' style='width:100%;height:"
					+ght+"px' marginwidth='0'  hspace='0' vspace='0' frameborder='0' scrolling='yes'></iframe>";
		}
		viewTabs.init(divid);
		var itemTab = viewTabs.tabs.getItem(tabid);
		if (!itemTab){
			var tabPanel = viewTabs.tabs.add({
				...
				html:html,
				listeners:{
　　　　　　　　　　　　　　activate:function(){ucapCommonFun.setIframeViewHeight();}
　　　　　　　　　　　},
				closable:this.closeabled
	        });
	        if(index==0)tabPanel.show();
	        if (type==2) {
				Ext.getDom('ifr'+tabid).src = url;
	        }
		} else {
			//itemTab.setTitle(title);
			itemTab.show();
		}
		if (type==1) {
		   // 说明是普通页面方式
	 ...
	 * 只提供给视图用
	 */
	addTab:function(divid,viewId,title,index,bModuleUnid){
		...
		this.commonAddTab(divid,url,2,viewId,title,index);//视图都是以iframe方式打开
	},
	init:function(renderto){
		if($(renderto).innerHTML!="" && viewTabs.isInit){
 ...
 */
function initView(viewId,renderto,title,hasTab,purl,bModuleUnid,outTbarHeight,noQuery,noPreview,noSelfConfig) {
	//var hasShow = false; 
	...
	if(undefined==hasTab || !hasTab){
		...
		for(var i=0;i<viewIds.length;i++){
			...
			if(i<titles.length){
				viewTabs.addTab(renderto,tmpViewId,titles[i],i,bModuleUnid);
			}else{
				viewTabs.addTab(renderto,tmpViewId,title,i,bModuleUnid);
			}
...
var viewConfigFun={
	...
	loadItemsConfig:function(viewId,listName){
		...
		if(undefined!=items){
			...
			//加入一个操作列，以方便应用系统进行配置
			if(listName=="vwItemList")ucapCommonFun.addOption(itemList,"~display~opr~","操作列");
		}
	},
	 ...
	 */
	loadButtonsConfig:function(btntype,listName,belongToModuleId){
		viewConfigFun.buttonConfigList = new Array();
		var url =ucapSession.baseAction;
		url+="?btntype="+btntype+"&type=subButton&action=getButtons&belongToModuleId="+belongToModuleId;
		...
		for(var i=0;i<viewConfigFun.subButtonItems.length;i++){
			...
			if(json.buttonUnid==obj.value){
				var field=["buttonName","buttonUnid","name","display","picture","useScopes",
					"unuseScopes","interaction","js","altKey","menu","tip","displayPosition"];
				var display="0";
				if(undefined!=json.display && (json.display==true || json.display=="1")){
					display = "1";
				}			
				var value=[json.buttonName,json.buttonUnid,json.name,display,json.picture,json.useScopes
					,json.unuseScopes,json.interaction,json.js,json.altKey,json.menu,json.tip,json.displayPosition];
				//alert(Ext.util.JSON.decode(json));
				var jsonValue ="{"+ucapCommonFun.getJsonValue(field,value)+"}";	
				ucapCommonFun.bindForm(jsonValue);
				break;
...
var ucapDept = {
	...
	setUserView:function(deptUnid){		
		...
		initView(viewId,"userView","","","deptUnid="+deptUnid,26);
...
//			alert(unid+"  "+ucapDictConfig.rootUnid )
			...
			treeloader.baseParams ={type:"dict",act:"getDictTree",flag:"edit",unid:unid,curAppUnid:ucapManagerTree.curBelongToAppId};
         }, this);
...
/*	    Ext.getCmp("dictFileNew").setDisabled(false);
	...
	save:function(msgName,newflag){
		...
		if (!isRename ){
			...
			var belongToApp=ucapManagerTree.curBelongToAppId;
			if(belongToApp==""){
...
function selectDataSD(type,selNum,inputName,conValue,outputFunc,
	...
	if (showType=="00"){ //树型
		...
		ucapTreeSelect.rootName =showName;
		ucapTreeSelect.actionType=actionType;
		ucapTreeSelect.act =actionName;
	...
	createTree:function(type,conValue){
		...
		loader.on('beforeload', function(treeloader, node) {
			if (node.id==this.deptRootId) return;
				var belongToAppId = ucapCommonFun.getUrlParameter("belongToAppId");
				if(belongToAppId){
				treeloader.baseParams ={type:"treeSelect",action:"getDeptTree",unid:node.id,selectType:type,belongToAppId:belongToAppId}
				}else{
         ...
         var listeners = {
         	dblclick : function(node){
         		if(listSelect.currentType=="200"){
         			if(!node.isLeaf())return;
         		}
         		if(listSelect.selNum==1)listSelect.delAll();
         		var obj={value:node.id,innerText:node.text};
...
//		}
	...
	treeAddSelect:function(){
		...
		for(var i=0;i<nodes.length;i++){
			...
			}else if(listSelect.currentType=="200"){
		...
		loader.on('beforeload', function(treeloader, node) {
         ...
         var listeners = {
         	dblclick : function(node){
         		...
         		var obj={value:node.attributes.value,innerText:node.text};
         		listSelect.addOptionByValue(obj);
         	}
         }
		var tree=new Ext.tree.TreePanel({
	...
	getProxool : function() {
		var requestConfig = {
			...
			params : {
				"type" : "changeDB",
				"action" : "getProxool",
				"alias" : ucapCommonFun.getUrlParameter("unid")
			},
			callback : function(options, success, response) {
				if (success) {
					var exjson = Ext.util.JSON.decode(response.responseText);
...
var proxoolFun = {
	 ...
	 * @param {} json
	 */
	valModify : function(deptName,json,win) {
		...
		proxoolFun.doModify(json,win);

	},

	doModify : function(json,win) {
 ...
 */
var appAssign = {
	initAssign:function(unitId,unitAdminId,deptName){
...
//				appAssign.selectConfirm(deptName);
	...
	selectConfirm:function(deptName){
		...
		var button=[
			{text:"确定",handler:function(){
				window.top.proxoolFun.valModify(deptName,"",win);
				}},
			{text:"取消",
					handler:function(){win.close()}
		}];
...
var permission={
    ...
    createTree:function(){
		...
		loader.on('beforeload', function(treeloader, node) {
			...
			treeloader.baseParams ={type:"permission",action:"getTarget",roleId:roleId,unid:unid,nodeType:nodeType,belongToApp:curAppUnid}
         }, this);
	...
	permissionConfig:function(){
		...
		var html="sys/cfgJsp/permission/permission.jsp?unid="+unid+"&formId="+formUnid;
		var button=[
					{text:"确定",handler:function(){permission.permissionConfirm()}},
					{text:"取消",
					handler:function(){ucapSession.commonWin.close()}
					}];
		ucapSession.commonWin = new Ext.Window({
			title:ucapSession.win.winImg+"权限配置",
		    width:510,
		    closable:true,    //取消
		    modal: true,     
			height:470,
	...
	permissionConfirm:function(){
		...
		var nodes = tree.getChecked();	
		var allNodes = permission.getAllNodes(tree);
		...
		var json = "{systemIds:'"+systemIds+"',menuIds:'"+menuIds+"',moduleIds:'"+moduleIds+"',allSystemIds:'"+allSystemIds+"',allMenuIds:'"+allMenuIds+"',allModuleIds:'"+allModuleIds+"'}";
		...
		var requestConfig = {
			url:ucapSession.baseAction,
			params:{"type":"permission","action":"save","roleId":roleId,"tmp":ucapCommonFun.getRandomString()},
			jsonData:json,
			callback:function(options,success,response){
				if (success){
					var exjson = Ext.util.JSON.decode(response.responseText);	
					...
					//Ext.Msg.alert("提示","视图排序保存成功！");
					Ext.Msg.alert("提示","权限配置保存成功！");
				} else {
					Ext.Msg.alert("提示","权限配置保存不成功，可能为网络断开连接！");
					ucapSession.commonWin.close();
				}
			}
		}
...
ucapManagerTree = {
	...
	openView:function(node,sourceUnid){
		...
		//alert(purl);id, type, openType, title,purl,moduleUnid
		ucapCommonFun.indexOpen.openView(viewId,1,node.text,purl);
	...
	//新建模块
	newModule:function(formId,formType,height,width){
		var node = ucapManagerTree.contacterTree.getSelectionModel().getSelectedNode();
		var obj = {};
	...
	filterBtn:function(node){
		...
		var fn = function(str){
			...
			if(btns)Ext.each(btns,function(btn){
				btn.disabled=false;
				if(btn.getAttribute("onck")!="1"){
					btn.setAttribute("onck","1");
					switch(btn.getAttribute("btnType")){
						case "01":{
						...
						case "03":{
							Ext.get(btn).on("click",function(){ucapManagerTree.newModule('02303CFE7296D94BF137A2B0CBF94EE1','02',200,550);});
							break;
						}
						case "04":{//刷新
		...
		var edit = (node?node.attributes.edit:"");
		if(edit!="1")str+=",[btnType=06],[btnType=07]";
		fn(str);
	}
};
...
ucapPopedomTree = {
	...
	init:function(funid){
		...
		if(!selectedRow || selectedRow.length!=1){
			Ext.Msg.alert("系统提示","必须且只能勾选选择一个选项");
			return;
		}
		var row = selectedRow[0];
		var roleId = row.data[grid.getColumnModel().getDataIndex(1)];
							...
							if (success){
								...
								if((flag+"")!="false"){
									...
									if(json && json.userUnid && json.deptUnid){
										...
										appAssign.initAssign(json.deptUnid,json.userUnid,json.deptName);
									}
								}
							}
					}
