	...
	if(StringUtils.isEmpty(actionUrl)){
		...
		if(null!=paraMap && !paraMap.isEmpty()){
			...
			while(itr.hasNext()){
				...
				String val = request.getParameter(keyName);
				String split = "&";
				if(StringUtils.isEmpty(paraUrl)){
					split = "?";
				}
				paraUrl += split+keyName+"="+val;
			}
			actionUrl += paraUrl;
		}
	}
...
<!-- 
    ...
    //应用建模：界面建模的子菜单、快捷方式、导航栏、界面方案和频道配置页面引入必要的js文件	
-->
...
<script type="text/javascript" src="<%=systemPath%>/ADMIN/default/view/js/view.js"></script>
...
//2011-12-12 add by xhuatang@linewell.com 添加顶部窗口的注册事件
if(!window.top.regEvents){
    window.top.regEvents = {};    
}
window.top.regEvents.refresh = function(){viewImp.refreshView();};
...
<body onload="loadViewDataPosition()">
	...
	<!-- 搜索栏 begin-->
	<form id="queryForm" name="queryForm" action="<%=actionUrl%>" method="post">
		...
		<input type="hidden" name="actionUrl" id="actionUrl" value="<%=WebAppUtils.convertNull(actionUrl)%>" />
