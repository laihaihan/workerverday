...
if(sAction.equals("UPDATECONFIG")){
	...
	s_FormDefaultLanguage = GetSelect("d_defaultlanguage", split("简体中文|繁体中文|英文|日语|西班牙语|俄语|德语|法语|意大利语|荷兰语|瑞典语|葡萄牙语|挪威语|丹麦语", "|"), split("zh-cn|zh-tw|en|ja|es|ru|de|fr|it|nl|sv|pt|no|da", "|"), sDefaultLanguage, "");
	...
	for(int i=1; i<aButton.length; i++){
		if (aButton[i][1].toUpperCase().equals(s_Code.toUpperCase())){
			if(aButton[i][5].equals("0")){
				...
				if(aButton[i][1].equals("FontName")){
					s_FixedWidth = " style='width:115px'";
				}else if(aButton[i][1].equals("FontSize")){
					s_FixedWidth = " style='width:55px'";
				}else if(aButton[i][1].equals("FormatBlock")){
					s_FixedWidth = " style='width:90px'";
				}else if(aButton[i][1].equals("ZoomSelect")){
					s_FixedWidth = " style='width:55px'";
				}else{
					s_FixedWidth = "";
				}
				...
				if (!aButton[i][4].equals("")){
					s_Result = "<SELECT" + s_FixedWidth + " CLASS=" + aButton[i][7] + " onchange=\\\"" + aButton[i][6] + "\\\">" + aButton[i][4] + "</SELECT>";
				}else{
					s_Result = "<SELECT" + s_FixedWidth + " CLASS=" + aButton[i][7] + " onchange=\\\"" + aButton[i][6] + "\\\">\"+lang[\"" + aButton[i][1] + "\"]+\"</SELECT>";
				}
