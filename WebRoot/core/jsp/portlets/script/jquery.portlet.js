    ...
    $.widget("ui.portlet", {
        ...
        _create: function() {
            ...
            $.each(o.columns, function(ci, c) {
                ...
                $.each(c.portlets, function(pi, p) {
                    ...
                    //关闭title.prepend("<a href='#' class='ui-corner-all ui-portlet-event'><span class='ui-icon ui-icon-closethick ui-portlet-close'></span></a>");
        ...
        _sortable: function(value) {
            ...
            var st = $(".ui-portlet-column", this.element).sortable({
                ...
                cancel: ".ui-portlet-content",
                //add by lijx......start.....
				stop: function( event, ui ) {
					var sort = _this.sort.call(_this,event,ui);
					var o = _this.options;
					if($.isFunction(o.getSort)) {
		                o.getSort(sort);
		            }
				}
				//............end
...
//            this._hoverable(refresh.parent());
        ...
        /**
         * sort//add by lijx......start.....
         **/
         sort: function(event,ui) {
         	var sortStr = "'',";
			$.each($(".ui-portlet-column"), function(m) {
				 //sortStr += $(this).attr('id') + ":";//三列的id
				 $.each($(this).children(".ui-portlet-item"), function(d) {
					 sortStr += "'"+$(this).attr('id') + "',";//对其进行处理，把每一列有的div的id分别储存起来。
				 })
				 sortStr =sortStr.substring(0,sortStr.length-1);
				 sortStr += "~@~'',";
			}); 
			sortStr = sortStr.substring(0,sortStr.length-6); //扣掉最后一个~@~'',
			return sortStr;
         },
		//.......................end...........
            ...
            if(type == 'text') {
                ...
                $.ajax({
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        var content = "<span style='padding:0.2em' class='ui-state-error ui-corner-all'>页面出错！</span>";
                        $(ct).html(content);
