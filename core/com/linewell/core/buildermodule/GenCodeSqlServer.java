...
public class GenCodeSqlServer  implements BuilderModuleGenCode{
	...
	public String getFieldSqlByTableNoUnid(String tableName) {
		...
		sb.append(" on a.id=d.id  and  d.xtype='U' and d.name<>'dtproperties' left join syscomments e  ");
		sb.append(" on a.cdefault=e.id left join sys.extended_properties g on a.id=g.major_id AND ");
		sb.append(" a.colid = g.minor_id where d.name='"+tableName.toUpperCase()+"' and a.name<>'UNID' order by a.id,a.colorder ");
		return sb.toString();
