...
$(function(){
	...
	if($('#AeroTaskbar').length==0&&AeroWindow.ShowTaskbar){
		$('body').append('<div id="AeroTaskbar" style="display:none" class="tskbg-hori horiBottom"><div id="StartMenu-Btn"></div><div id="TaskbarSeparate"></div><div title="Show Desktop" id="TaskbarShowDesktop"></div></div>');
	...
	if(AeroWindow.ShowTaskbar)
	{
		...
		stmenu.load('core/js/aerowindowdevelopment/ajaxload/StartMenu.jsp',function(){$('#NavItemsCenter').html(list);});
	...
	$('#StartMenu-Btn').live("click",function(){
		if($(this).hasClass('active')){
			...
			stmenu.show(200);}});
	$('#rwltop').click(function(){
		$('#AeroTaskbar').removeAttr('class');
		$('#AeroTaskbar').addClass('tskbg-hori horiTop');
	...
	$('#rwlbottom').click(function(){
		$('#AeroTaskbar').removeAttr('class');
		$('#AeroTaskbar').addClass('tskbg-hori horiBottom');
   ...
   function transparentalloff(){
	...
	$('html').click(function(event){
		...
		var outClicked_startwindow=true;
		...
		if($(event.target).parents('#starttable').length >0 ){
			outClicked_startwindow = false;
		}
		if(outClicked_startmenu&&outClicked_startwindow){
