      ...
      //top.location.href = url;
		...
		createGridDiv : function(index, render) {     
			...
			if (undefined == mainRight) {
				...
				//2012-08-06 mdf by chuiting@linewell.com
				//BUG1228：ADMIN中心：所有的视图界面都有问题，视图高度只有100
				var clientHeight = document.compatMode=="CSS1Compat" ? document.documentElement.clientHeight : document.body.clientHeight;
				mainRight.style.height = clientHeight - 36 - 93;
				//end 2012-08-06 mdf by chuiting@linewell.com
		...
		loadCommonGrid : function(curViewId, index, noPreview, noSelfConfig) {
			...
			// modify by zzhan@linewell.com 20120208 如果视图有搜索框则grid减去搜索框高度
			if(searchDiv){
				var display = searchDiv.style.display;
				if("none"==display){
					ucapUmc.otherViewHeight = 0;
				}
			}
			// modify end
			if (view.viewBaseInfos[index].isEdit) {
				grid = new Ext.grid.EditorGridPanel({
							...
							sm : sm,// 复选框
							loadMask : true, //
							height : Ext.get("ucapView").getHeight()-ucapUmc.otherViewHeight,
        ...
        if(index==-1){
          ...
          var fn = function(viewMId,wins){
            ...
            for (var i = 0; i < iframes.length; i++) {
              ...
              if(ifrWin.view && ifrWin.view.personal.getIndexFromBaseInfo){
                ix = ifrWin.view.personal.getIndexFromBaseInfo(viewMId);
                if(ix==-1){
