...
static String GetRndFileName( String sFileExt, String sPath,String fileName){
	int i = (int)(Math.random()*900) + 100;
	String sFileName = formatDate(new Date(), 4) + String.valueOf(i) ;

	java.io.File f = new java.io.File(sPath+sFileName);
	if (f.exists()){
		return GetRndFileName(sFileExt,sPath,fileName);
	}
	f.mkdirs();
...
if (sAction.equals("REMOTE")){
	...
	if (!sAllowExt.equals("") && !sRemoteContent.equals("")){
		...
		for(int i=0; i<a_RemoteUrl.size(); i++){
			...
			String fileName=sRemoteurl.substring(sRemoteurl.lastIndexOf("/")+1);
			SaveFileName = GetRndFileName(SaveFileType, sRealUploadPath,fileName);
	...
	try {
		...
		sSaveFileName = GetRndFileName(sFileExt, sRealUploadPath,sOriginalFileName);
