...
<%@page import="com.linewell.ucap.admin.module.ButtonWrapper"%>
...
List<Button> btnList = ButtonWrapper.getButtonByModuleUnid(moduleUnid);
int btnCount = (null == btnList ? 0 : btnList.size());
%>  
<!DOCTYPE html>
<html>
  <head> 
...
var formUnid = "4BE72EDFF3EC4E28B729373A00E32CD4";
...
function openFormPage(unid){
	var formUrl = "<%= systemPath %>sys/jsp/document.jsp?unid="+unid+"&type=01&formId="
   					+ formUnid + "&viewMId=4D45FAF515521727878953D858C7D682&openST=";											
...
function deleteForm(unid){
    ...
    var postData ="{\"formUnid\": '4BE72EDFF3EC4E28B729373A00E32CD4',\"formType\":\"01\",\"docUnid\":'"+unid+"',\"strResult\":\"\",\"isFlowItem\":false,\"belongToAppId\":'"+appUnid+"'}";  
	// 提交到后台
    $.ajax({
       type : "post",
       url : actionUrl,
	    ...
	    if(!isNormal){       
	       ...
	       $(".formCfgBtn").each(function(){
	           ...
	           $cancleIcon.click(function(){
	               if(confirm("此操作无法恢复！确定要删除选择的按钮吗？")){
	                    deleteForm(unid);
	               }
	           });
  ...
  <div>
	...
	<!-- 工具栏区域 -->
		<div class="formCfgToolBar">
			...
			<h3><%= btnCount%></h3>
			<h2>个按钮</h2>
			<input class="toolBarAdd formAdd" name="btnButtonAdd" id="btnFormAdd"type="button" value="添加" title="添加按钮"/>
			<input class="toolBarMg formManage" name="btnButtonManager" id="btnFormManager" type="button" value="管理" title="已有按钮删除管理" />
		...
		<div class="formCfgBtns">
			...
			%>
		   	<div class="formCfgBtn" unid="<%=btn.getUnid()%>">
				<img alt="" src="../module/style/images/module/designIcon_6.png" class="btnImg"/>
				<a><%=btn.getName()%></a>
			</div>	    
			<%}}%>		
		</div>
